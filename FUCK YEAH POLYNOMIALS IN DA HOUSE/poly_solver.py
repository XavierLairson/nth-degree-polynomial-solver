import poly_precision as precision
import sys
            ...
            for index in range(len(zeroes)):
                ...
                if abs(evaluate_poly(poly, CP)) <= small:
                    ...
                    if index == 0:  # left-most CP
                        ...
                        adjacent_check = True
                        if abs(evaluate_poly(poly, zeroes[index + 1])) < 0:
                            adjacent_check = False
                        right_CP_check = True
                        best_guess = small + CP
                        start = POI[index]
                        POI_count = 1
                        while (abs(start - CP) < small or start < CP) and right_CP_check and index + POI_count < len(POI):
                            start = POI[index + POI_count]
                            POI_count += 1
                        # one of the in-betweens (i died)
                        if (evaluate_poly(poly, CP_right) - evaluate_poly(poly, CP)) * evaluate_poly(derivative(poly), best_guess) > 0 and right_CP_check and adjacent_check:
                            best_guess = tangent_zero(
                                nth_derivative(poly, 2), start)
                        ...
                        while (abs(start - CP) < small or start < CP) and right_CP_check and index + POI_count < len(POI):
                            start = POI[index + POI_count]
                            if start == zeroes[index + 1]:
                                right_CP_check = False
                            POI_count += 1
                        # one of the in-betweens (i died)
                        if (evaluate_poly(poly, CP_right) - evaluate_poly(poly, CP)) * evaluate_poly(derivative(poly), best_guess) > 0 and right_CP_check and adjacent_check:
                            best_guess = tangent_zero(
